...
public class task_6 {
    ...
    public static String translateSentence(String line)// перевожу предложение в "свинский латинский"
    {
        ...
        if ((line.charAt(line.length() - 1) == '.' && line.indexOf(".") == line.length() - 1) || (line.charAt(line.length() - 1) == '!' && line.indexOf("!") == line.length() - 1) || (line.charAt(line.length() - 1) == '?' && line.indexOf("?") == line.length() - 1)) {//проверяю, есть ли одна "." и стоит ли она в конце
        ...
        for (String cPart : line.split("="))// разбиваю строку на части
        {
            ...
            if (isCorrect) {
                ...
                for (char c : cPart.toCharArray()) {
                    if (Character.isDigit(c)) { // запоминаю значения в формуле
                        if (move == 0)
                            v1.append(c);
                        else
                            v2.append(c);
    ...
    // неповторяющуюся подстроку для ввода строки
    {
        ...
        for (int i = 1; i < line.length(); i++) {
            if (cLine.indexOf(line.charAt(i)) != -1) {//checks recurring char
